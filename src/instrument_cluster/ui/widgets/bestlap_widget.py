...
class BestLapWidget(DirtySprite):
    ...
    def __init__(
        ...
        *,
        ...
        self.font_value = load_font(size=64, family=FontFamily.D_DIN_EXP_BOLD)
        ...
        self.set_lap("00:01.00")  # initial placeholder
        self.visible = 1
        self.dirty = 2
    ...
    def set_lap(self, time: float):
        speed_str = "0" if time is None else str(time)
    ...
    def update(self, packet: TelemetryFrame | None, dt: float):
        best_lap_time = float((getattr(packet, "best_lap_time", 0.0) or 0.0) * 1e-3)
        self.set_lap(self.format_mm_ss_hh(best_lap_time))
